...
public class TalkBubble extends Sprite {
	...
	private var type:String; // 'say', 'think', 'whisper', or 'shout'
	...
	private function setText(s:String):void {
		var desiredWidth:int = (type == 'shout') ? 250 : 135;
	...
	private function setWidthHeight(w:int, h:int):void {
		...
		if (type != 'think') drawTalk(w, h);
		else drawThink(w, h);
	}

	private function makeText():TextField {
		var format:TextFormat = new TextFormat('Verdana', 13, 0, true);
		if (type == 'shout') format = new TextFormat('Verdana', 28, 0, true);
		if (type == 'whisper') format = new TextFormat('Verdana', 13, 0, false, true);
